...
# Option prompt
action=$(zenity --list --title="Manage ZeroTier GUI" --text="Choose an action:" --radiolist --column "Select" --column "Action" TRUE "Install/Update" FALSE "Uninstall" --height=200 --width=300)

# If the user closes the window or hits "Cancel" (i.e., action is empty), exit
if [ -z "$action" ]; then
...
flatpak install --user -y flathub org.gnome.Platform/x86_64/49 2>&1 | tee /dev/null
